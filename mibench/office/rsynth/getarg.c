...
#endif
{
 ...
 while (i < argc)
 {
     ...
     if (*s && (flag == '-' || flag == '+'))
     {
         int off = 1, s_loop_stop = 0;
         while (*s && s_loop_stop == 0)
         {
             char *a;
             int count = 0;
#if defined(__STDC__)
             ...
             va_start(ap);
#endif
             int va_arg_loop_stop = 0;
             while ((a = va_arg(ap, char *)) && va_arg_loop_stop == 0)
             {
                 int l = strlen(a);
                 ...
                 else if (l > 1)
                 {
                     if (!count && !strcmp(s, a))
                     {
                         ...
                         count++;
                         s += l;
                         va_arg_loop_stop = 1;               /* out of va_arg loop */
                     }
                 }
                 else
                 {

...
char *context;                    /* last char of text to be matched */
{
    ...
    pat = pattern + (count - 1);
    text = context;
    static int done_loop = FALSE, result = TRUE;
    for (; count > 0 && !done_loop; pat--, count--)
    {
...
char *context;                    /* last char of text to be matched */
{
    ...
    pat = pattern;
    text = context;
    static int result = TRUE, done_loop = FALSE;
    for (pat = pattern; *pat != '\0' && !done_loop; pat++)
    {
        /* First check for simple text or space */
        if (isalpha(*pat) || *pat == '\'' || *pat == ' ')
        {
...
Rule(*rules);
{
    ...
    *output;
    ...
    for (;;)                         /* Search for the rule */
    {
        ...
        int for_loop_stop = 0;
        for (remainder = index; *match != '\0' && for_loop_stop == 0; match++, remainder++)
        {
